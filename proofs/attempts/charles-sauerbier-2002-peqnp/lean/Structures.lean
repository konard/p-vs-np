...
def is3SAT (cnf : CNF) : Prop :=
  ∀ c : Clause, List.Mem c cnf → is3SATClause c
...
def clearBit (b : Byte) (i : Nat) : Byte :=
  -- Clear the i-th bit by ANDing with the complement of 2^i
  let mask := 255 - (2^i % 256)  -- equivalent to ~~~(2^i) in 8-bit
...
def isUnsatCertificateA (a : StructureA) : Prop :=
  ∃ elem : AElement, List.Mem elem a ∧ isZeroByte elem.assignments

-- Certificate of non-satisfiability: some element has all constraints
def isUnsatCertificateD (d : StructureD) : Prop :=
  ∃ elem : DElement, List.Mem elem d ∧ isFullByte elem.constraints
